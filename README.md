...
Преобразование всех этих атрибутов в существующую разметку может быть утомительным! Разработчики React рекомендуют использовать [конвертер](https://transform.tools/html-to-jsx) для перевода существующего HTML и SVG в JSX. Конвертеры очень полезны на практике, но все же стоит понимать, что происходит, чтобы можно было комфортно писать JSX самостоятельно.
