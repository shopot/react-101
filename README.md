...
Если посмотреть на реализацию и использование действий (action) для приложения "Todo App", то можно увидеть, что в двух
местах используются явно заданные (hardcoded) повторяющиеся значения типа `add_new_todo`,  `remove_todo`
и `toggle_completed` их можно заменить на строковые константы.

Определение типа экшена в виде констант является наследием архитектуры **Flux**, во **Flux** традиционно считается, что
вы должны определять каждый тип экшенов строковой константой, сам список констант обычно выносится в отдельный модуль:
...
- Список типов экшено, которые уже были добавлены, удалены и изменены в Pull Request поможет каждому члену команды
...
вызываются, создавать функции, генерирующие их.
...
Вы можете написать генератор экшенов в отдельном модуле:
...
Затем импортировать его в компонент:
