...
* [Redux Toolkit на примере приложения Counter App](#redux-toolkit-на-примере-приложения-counter-app)
  ...
  * [Добавление асинхронной логики в Counter App](#добавление-асинхронной-логики-в-counter-app)
...
- **Redux Thunk** - это middleware для Redux, который позволяет вам создавать асинхронные действия с использованием функций. Он достаточно популярен и может быть легко интегрирован с Redux Toolkit, в последних версиях Redux Toolkit интегрирован по умолчанию и устанавливается автоматически в качестве зависимости.
...
Эти дополнительные объекты действий могут быть использованы для обновления состояния приложения, отображения состояния загрузки или отображения сообщений об ошибках.

Функция `createAsyncThunk()` принимает два обязательных параметра и один необязательный параметр:

- **type** - Тип действия, строка в нотации `domain/actionInCamelCase`, тип используется для создания уникальных типов действий для каждого этапа асинхронной операции (например, "pending", "fulfilled", "rejected").
- **payloadCreator** - Это функция обратного вызова (callback), которая выполняет асинхронную операцию и возвращает обещание (promise) с результатом операции. Эта функция может быть асинхронной или использовать асинхронные операции, такие как запросы к серверу или обращения к API. Она принимает два аргумента: `arg` (аргумент, переданный при вызове thunk-функции) и `thunkAPI` (объект, предоставляющий методы `dispatch`, `getState` и другие утилиты).
- **options** (необязательный) - Это объект с дополнительными параметрами для настройки поведения созданных thunk-функций. Возможные параметры включают `condition` (функция-условие, которая определяет, должна ли быть выполнена асинхронная операция), `dispatchConditionRejection` (флаг, указывающий, должно ли отклонение условия быть диспатчено как отклоненное действие), `extraReducers` (дополнительные редюсеры, которые будут обрабатывать действия, созданные `createAsyncThunk`) и другие.

Функция **payloadCreator** будет вызываться всегда с двумя аргументами:
...
### Добавление асинхронной логики в Counter App
