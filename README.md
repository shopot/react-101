...
- `ReducersMapObjectt`: это объект, значения которого соответствуют различным функциям-редюсерам, так называемые
  слайс-редюсеры (slice reducers).

Пример с двумя слайсами в виде ReducersMapObject (JavaScript):
...
Тот же пример с `combineReducers()` (JavaScript):
...
- `reducer`: одиночная функция-редюсер, которая используется как Root Reducer или объект состоящий из слайс-редюсеров (
  slice reducers)
...
- `preloadedState`: Начальное состояние, аналогичное начальному состоянию `createStore()`. Это можно использовать для
  ...
  сессии пользователя.
- `enhancers`: усилители, предоставляют возможность добавлять дополнительную функциональность к Redux Store. Это могут
...
`RootState` и `AppDispatch` берется из файла `src/store/store.ts` где вызывается функция `configureStore()`.

Фрагмент кода определением типов для Redux Store и dispatch:
...
Дополнительно можно инкапсулировать логику создав отдельную колбэк-функцию для передачи в хук `useSelector()` :
