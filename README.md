...
Функция `register()` возвращает объект, который содержит `onChange()`, `onBlur()`, `ref`, `name`, которые в свою очередь
...
- `minLength` - Минимальная длина значения текстовых данных (строк);
- `maxLength` - Максимальная длина значения текстовых данных (строк);
...
Вы можете просто передать свое сообщение об ошибке для `register()` через атрибут `message` объекта для правил валидации, а
...
использования выбранной библиотеки, и выполняет валидацию с использованием этой библиотеки для входных данных формы.

Полный список поддерживаемых библиотек, список resolver-функций и примеры использования можно найти
...
Поскольку `react-hook-form` использует неконтролируемые компоненты формы, вы можете указать `defaultValue`
...
🔗 [Ссылка на деплой приложения с примером формы](https://react-hook-form-ab1e50.netlify.app)

Готовый пример с приложением находится в src раздела chapter-18.
