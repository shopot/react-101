...
По умолчанию команда запуска `vitest` стартует в режиме **vitest watch**, в этом режиме Vitest следит за изменениями
файлов с тестами и перезапускает тесты при их изменении, данный режим работает только в **Development environment**, при
запуске в CI (Continuous Integration) Vitest будет игнорировать **vitest watch** и будет выполняться в режиме **vitest
run**, то есть запускаться один раз для всего набора тестов.
