...
В случаях, когда необходимо внедрение зависимостей, например, когда состояние должно быть инициализировано реквизитами из компонента, рекомендуемый подход — использовать стандартное хранилище с `React.Context`.

Пример реализации можно можно посмотреть в официальной документации:

https://docs.pmnd.rs/zustand/guides/initialize-state-with-props

В реальности, реализации данного кейса будет отличаться от того, что написано в документации, это связано с природой `React.Context`, лучшем вариантом будет использовать [Splitting code (No store actions pattern)](#splitting-code-no-store-actions-pattern) совместно с `React.Context`.
