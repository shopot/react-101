...
запросы к API, манипуляции с DOM-элементами, подписка и отписка на события и т.д.

`useEffect` — это хук, который можно использовать для замены некоторых методов жизненного цикла классового компонента.
useEffect используется в функциональных компонентах в следующих случаях:
...
Декларация API хука `useEffect`  на момент написания материала:
...
- `setup` (или setup function) - это функция обратного вызова с реализацией логики вашего эффекта, внутри которой
  происходит работа с обновленными данными. Эта функция может вернуть другую функцию, которая называется функцией
  очистки (сleanup function), внутри которой происходит работа с данными до обновления.
...
Таким образом, код в `useEffect` срабатывает после того, как компонент обновлен на экране. Если вы хотите выполнить
какую-то логику до первого рендеринга компонента, например, отправить запрос на сервер и получить данные, вы можете
передать вторым аргументом в useEffect пустой массив. Это сообщает React, что эффект должен быть выполнен только один
раз во время первого монтирования компонента, что можно сравнить с методом классового компонента `componentDidMount()`.
...
Если вы не передадите второй аргумент, побочный эффект в функции обратного вызова (setup function) будет срабатывать
снова при каждой визуализации компонента, что можно сравнить с методом классового компонента `componentDidUpdate()`.
...
Если вы передадите пропсы или переменные состояния во втором аргументе, побочный эффект в функции обратного вызова (
setup function) будет выполняться только при изменении значений пропсов или переменной состояния.
...
предоставили) со старыми значениями, а затем запускает функцию настройки (setup function) с новыми значениями. После
того, как ваш компонент будет удален из DOM, React то же запустит функцию очистки (cleanup function).
