...
- `current` - это значение, которое было установлено при первом вызове `useRef()`. Позже вы можете установить
  ...
  React установит этот элемент как текущее значение свойства `current`.
...
- В строгом режиме (Strict Mode) React дважды вызовет функцию вашего компонента, чтобы помочь вам найти случайные
  побочные эффекты. Это поведение предназначено только для разработки и не влияет на production-сборку. Каждый
  ref-объект будет создан дважды, но одна из версий будет отброшена. Если ваша функция компонента чистая (как и должно
  быть), это не должно влиять на поведение.
...
- Если входные данные (пропсы, состояние и контекст) одинаковы, то компонент будет возвращать точно такой же JSX.
...
🔗 [Ссылка на деплой приложения с примерами `useRef()`](https://react-useref-ab1e50.netlify.app)

Готовый пример с приложением находится в `src` раздела chapter-16.
