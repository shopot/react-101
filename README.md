...
- Оправка простых объектов `actions` через метод `dispatch` в `Store`, когда что-то происходит в приложении;
- Чистые функции рюдюсеры, отслеживающие эти действия и возвращающие _immutably updated_ состояние;
...
**Middleware** - это функция более высокого порядка, которая применяется к функции `dispatch function` для возврата
новой
...
написание много шаблонного кода в виде констант, генераторов, асинхронных действий, middleware и тд.
