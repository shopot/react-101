...
Redux как и Redux Toolkit **не умеет работать в асинхронном режиме**, в этом случае всю логика обработки запросов необходимо размещать внутри компонентов React.
...
При создании асинхронного thunk действия с помощью `createAsyncThunk()`, RTK автоматически генерирует имена дополнительных действий, таких как :
...
`createAsyncThunk()` принимает два аргумента: тип действия и функцию, которая выполняет асинхронную операцию. Функция должна возвращать промис, который будет разрешен или отклонен в зависимости от успешности или неуспешности операции.
