...
- [Компонент формы `<option>`](#компонент-формы-option)
- [Компонент формы `<select>`](#компонент-формы-select)
- [Компонент формы `<textarea>`](#компонент-формы-textarea)
- [Приложение `Todo App`](#challenge-приложение-todo-app)
...
- Форма для добавления задачи содержит управляемый компонент (элемент) `<input>` (использование useState внутри
  компонента)
- Форма для добавления задачи может содержать другие компоненты, общие для всего приложения (например `<Button />`)
- Компонент `<App />` управляет компонентом формы и списка Todo (реализует бизнес логику для списка Todo)
- Для хранения списка используется переменная состояния (useState) в `<App />`
