...
функцией редюсер `todos`, позже мы еще вернемся к настройкам к этом файлу для завершения настройки `store`:
...
Для создания функции редюсера используется функция RTK `createSlice()`, которая принимает три параметра:

- `name`: строковой параметр, имя, которое будет задействовано в действиях (actions) с использованием
