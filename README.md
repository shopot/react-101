...
* [Testing Library](#testing-library)
  * [Чем не является эта библиотека](#чем-не-является-эта-библиотека)
  * [Чего следует избегать при использовании Testing Library](#чего-следует-избегать-при-использовании-testing-library)
* [Queries - запросы](#queries---запросы)
  * [Типы запросов](#типы-запросов)
    * [Для одного элемента](#для-одного-элемента)
    * [Для нескольких элементов](#для-нескольких-элементов)
  * [Приоритет запросов](#приоритет-запросов)
    * [1. Queries Accessible to Everyone](#1-queries-accessible-to-everyone)
    * [2. Semantic Queries](#2-semantic-queries)
    * [3. Test IDs](#3-test-ids)
* [Vitest с React Testing Library](#vitest-с-react-testing-library)
* [React Testing Library - Рендеринг компонента](#react-testing-library---рендеринг-компонента)
* [React Testing Library - выбор элементов](#react-testing-library---выбор-элементов)
* [Поиск на основе запросов queryBy и findBy](#поиск-на-основе-запросов-queryby-и-findby)
  * [В чем разница между getBy и queryBy?](#в-чем-разница-между-getby-и-queryby)
  * [Когда использовать findBy?](#когда-использовать-findby)
  * [Функции сопоставления - Assertive functions](#функции-сопоставления---assertive-functions)
* [Запуск событий DOM - fireEvent](#запуск-событий-dom---fireevent)
* [Взаимодействие с пользователем - userEvent](#взаимодействие-с-пользователем---userevent)

## Testing Library

[@testing-library](https://testing-library.com/docs/)  - это семейство библиотек для тестирования пользовательского
интерфейса (UI) веб-приложений на JavaScript.

Основная библиотека `@testing-library` была создана для предоставления эргономичных API-интерфейсов для нескольких
платформ, включая React, Angular и Vue. Она предоставляет инструменты для поиска элементов страницы и взаимодействия с
ними. Существует также плагин для использования запросов к библиотеке тестирования для сквозных тестов в Cypress и
реализация для React Native.

`@testing-library` имеет отдельные библиотеки для каждого фреймворка, которые предоставляют специальные утилиты и
функции для работы с этим фреймворком. Например:
...
Запросы (Queries) - это методы, которые библиотека `@testing-library` предоставляет вам для поиска элементов на
странице.

Существует три вида запросов:

- **get** - этот вид запросов предназначен для поиска элемента, который возвращает соответствующий узел для запроса.
- **query** - этот вид запросов предназначен для поиска элемента, который возвращает значение `null`, если ни один
  элемент не соответствует. Запрос **query** может использоваться для проверки отсутствия элемента.
- **find** - этот вид запросов возвращает Promise, который разрешается, когда найден элемент, соответствующий
  данному запросу.
...
Запрос `getByText()` может принимать в качестве аргумента строку, а также регулярное выражение.
...
Запрос `getByText()` - это лишь один из многих запросов поиска в библиотеке RTL.
...
Чтобы подтвердить отсутствие элемента, можно заменить **getBy** на **queryBy**:
...
- 🔗 [Testing Library: Firing Events API](https://testing-library.com/docs/dom-testing-library/api-events)
- 🔗 [React Testing Library](https://testing-library.com/docs/react-testing-library/intro) 
- 🔗 [jest-dom's documentation](https://github.com/testing-library/jest-dom)
