...
Это позволит отделить действия от самого стора и тогда изменение состояния стора  внутри контекста не будет вызывать повторный рендеринг компонентов, которые будут использовать только действия для мутации состояния без самих переменных состояния.
