...
#### Выражение типа функции и сигнатура вызова (Call Signatures)

Самый простой способ описать функцию (создать типа для функции) - использовать выражение типа функции. 
Эти типы синтаксически похожи на стрелочные функции:

```ts
// Выражение типа функции
type MyFunctionType = (a: number, b: number) => number;

// Пример использования выражения типа функции
const myFunction: MyFunctionType = function(x, y) {
  return x + y;
};

// Вызов функции
const result = myFunction(2, 3); // Результат: 5
```

Синтаксис `(a: number, b: number) => number` означает, что функция принимает два аргумента типа `number` и возвращает значение типа `number`.

Следующий пример описывает функцию при помощи сигнатуры вызова в типе объекта:

```ts
// Сигнатуры вызова в типе объекта
type MyFunctionType = {
  (a: number, b: number): number;
}

// Сигнатуры вызова в интерфейсе
interface MyFunctionType {
  (a: number, b: number): number;
}
```

Если вы хотите описать что-то вызываемое с помощью свойств, вы можете использовать сигнатуры вызова:
...
Перегрузка функций позволяет определять несколько вариантов функции с различным количеством и типами аргументов, чтобы обрабатывать различные типы входных параметров и возвращаемых значений.

**Сигнатуры перегрузки** - это описания функции с указанием типов входных параметров и возвращаемого значения, но без тела функции. Все перегрузки должны иметь одинаковое имя, и последующие перегрузки должны быть более конкретными, чем предыдущие.

**Сигнатура реализации** - это фактическое определение функции с телом, которое соответствует одной из сигнатур перегрузки.
...
В этом примере две сигнатуры перегрузки функции `createElement` и сигнатура реализации: 
- Первая принимает строку "input" и возвращает `HTMLInputElement`.
- Вторая принимает строку "img" и возвращает `HTMLImageElement`.
- Третья принимает любую строку и возвращает `HTMLElement`.

Все сигнатуры перегрузки должны иметь одинаковое имя, а сигнатура реализации должна быть совместима со всеми сигнатурами перегрузки и определять поведение функции. 
...
Объявляя значение по умолчанию для параметра универсального типа, делает необязательным явное указание типа аргумента.
...
let box1: Box<number> = { value: 10 }; // Тип T устанавливается в number
let box2: Box = { value: 'Hello' }; // Тип T устанавливается в string по умолчанию

let box3: Box = { value: true }; // Ошибка: Type 'boolean' is not assignable to type 'string'.
```

В этом примере `T = string` устанавливает параметр `T` по умолчанию в `string`. Это означает, что если тип `T` не указан явно при создании объекта `Box`, то он будет установлен в `string`.

Пример использования параметров по умолчанию:

```ts
type Pair<F, S = number> = {
  first: F;
  second?: S;
};

let pair1: Pair<string, boolean> = { first: 'hello', second: true }; // Оба параметра явно указаны
let pair2: Pair<string> = { first: 'world' }; // Параметр S не указан, используется значение по умолчанию (number)
```

Параметры типа (Общий параметр) по умолчанию подчиняется следующим правилам:
...
- При указании аргументов типа вам необходимо указать аргументы типа только для обязательных параметров типа. Параметры неуказанного типа будут преобразованы в типы по умолчанию.
- Если указан тип по умолчанию и вывод не может выбрать кандидата, выводится тип по умолчанию.
- Объявление класса или интерфейса, которое объединяется с существующим объявлением класса или интерфейса, может ввести значение по умолчанию для существующего параметра типа.
- Объявление класса или интерфейса, которое объединяется с существующим объявлением класса или интерфейса, может содержать новый параметр типа, если он указывает значение по умолчанию.
